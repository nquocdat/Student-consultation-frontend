...
export default function StaffSidebar() {
    ...
    {/* --- 2. Thông báo chung (ĐÃ SỬA) --- */}
    <li className="nav-item">
        <NavLink
            to="/staff/notifications"
            // Đồng bộ logic className
            className={({ isActive }) => isActive ? "nav-link active-nav" : "nav-link"}
            // Đồng bộ logic style
            style={({ isActive }) => isActive ? { ...styles.navLink, ...styles.activeLink } : styles.navLink}
        >
            {/* Đồng bộ icon (me-3, fs-5) */}
            <i className="bi bi-bell-fill me-3 fs-5"></i>
            <span>Thông báo chung</span>
        </NavLink>
    </li>

    {/* --- 3. Lịch công tác (Đang khóa) --- */}
    <li className="nav-item">
        <NavLink
            to="/staff/schedule"
            className="nav-link"
            style={{ ...styles.navLink, opacity: 0.5, cursor: "not-allowed" }}
            onClick={(e) => e.preventDefault()} // Ngăn không cho click vào link bị disable
        >
